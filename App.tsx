    ...
    for (const rpcUrl of rpcUrls) {
      ...
      try {
        ...
        const parsedLogs: (Receipt | null)[] = await Promise.all(logs.map(async (log: any) => {
          ...
          // Check if this proof was previously masked
          let isRevealedState = true; // Default to revealed for new proofs

          // Check local receipts first
          const localReceipts = localStorage.getItem('proofly_receipts_v1');
          if (localReceipts) {
            try {
              const parsed = JSON.parse(localReceipts);
              const existing = parsed.find((r: Receipt) => r.id === log.transactionHash || r.hash === proofHash);
              if (existing && existing.isRevealed !== undefined) {
                isRevealedState = existing.isRevealed;
              }
            } catch (e) { }
          }

          // Check cached global receipts if not found in local
          if (isRevealedState === true) {
            const cachedGlobal = localStorage.getItem('proofly_global_cache');
            if (cachedGlobal) {
              try {
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-[var(--brand-gray)] text-slate-700 selection:bg-[var(--brand-blue-glow)] selection:text-[var(--brand-blue)] overflow-hidden">
      ...
      `}>
        ...
        <div className="flex items-center justify-center gap-4 pt-4 border-t border-white/5">
          ...
          <a onClick={() => { setView('docs'); setIsSidebarOpen(false); }} className="text-slate-400 hover:text-emerald-400 transition-colors cursor-pointer" title="Documentation">
